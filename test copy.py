...
def parse_dt(s: str) -> datetime:
    """Acepta ‘2025-05-01’, ‘2025-05-01T14:30’, etc. Devuelve datetime tz-aware (Madrid)."""
...
ENTITY_ENDPOINT = {
    "tasks":        ("tasks",        task_from_api,        "task_id"),
    "issues":       ("issues",       issue_from_api,       "issue_id"),
    "epics":        ("epics",        epic_from_api,        "epic_id"),
    "userstories":  ("userstories",  userstory_from_api,   "userstory_id"),
    }
...
def main(argv: list[str] | None = None):
    ...
    ap.add_argument("--events", default="tasks,userstories,issues,epics",help="List separated by commas pf the events to backfill, by default: tasks,userstories,issues,epics")
    ...
    for event in events: # Iterate over the events to backfill
        ...
        docs = [converter(r, ns.project) for r in raw]        # Convert the raw data to the MongoDB schema using the converter function
        coll = get_collection(f"taiga_{ns.project}.{event}")  # Get the MongoDB collection for the event
